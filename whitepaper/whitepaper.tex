...
\title{Multilateral Token Trade Protocol (MTTP)\\一个以太坊代币的多边交易协议\\（草稿 v0.2）}
\author{
    dong77@gmail.com\\
%    \and
...
%在章节\ref{sec:fee}中，我们还会详细介绍交易所通过MTC代币抵押的原因和细节，最终的结果是每个交易都被迫给自己的撮合的实际兑换率打个折扣，即\texttt{交易所折扣}。假交易所$X$的交易所折扣为$\mu$，那么由该交易所撮合是，最后的实际成交兑换率为：$\hat{r^i} = r^i \cdot (1-\gamma) \cdot (1-\mu)$
...
message Order {
  ...
  unit256 fee;        // 交易总费用（$MTC$ Token），部分成交的费用按该次
