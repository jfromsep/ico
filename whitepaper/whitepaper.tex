...
在下一章节，我们还引入了一个激励交易所竞争的代币抵押机制，智能合约根据每个交易所代币抵押排名，为每个交易所计算一个分润折扣，$\lambda$，该折扣会计入交易所的费用收取计算上；同时，交易所也可以指定为某个交易环路做进一步的费用优惠折扣，$\eta$。因此，交易所对于一个交易环路的整体费用为：
...
$$\lambda_{n} = 0.05*(\ln (n+e-1) - 1)\text{。}$$
具体返利比例如下表所示：
\begin{table}
\caption{抵押MTX排名与分润折扣} %显示表格的标题
\centering
\begin{tabular}{p{3.5cm}|p{3cm}} %设置了每一列的宽度，强制转换。
\hline
抵押MTX排名 & 分润折扣$\lambda$ \\ %用&来分隔单元格的内容 \\表示进入下一行
\hline %画一个横线，下面的就都是一样了，这里一共有4 行内容
1 & 0\%\\
\hline
2 & $1.57\%$\\
\hline
...
对于排名1001及以后的交易所，或未进行抵押的交易所，统一实施30\%强制分润折扣。

如下图所示，折扣的下降并非线性的，如$\lambda_{2} - \lambda_{1} \gg \lambda_{100} - \lambda_{99}$。
